...
//    }
    ...
    public void addEmployee(String name, double salary, int state) {
        ...
        if (checkIsArrayFull(true)) { // Проверяем, полный ли массив, и если да, то
            ...
            System.out.println();
        } else { // если же он не полон

            // проверка, есть ли уже такая запись в массиве, чтобы избежать дублирования данных
            Employee newEmployee = new Employee(name, salary, state, idTemp); // заносим во временную переменную данные, полученные на входе, плюс в качестве id поставляем число из временной переменной
            for (int i = 0; i < employees.length; i++) { // задаем цикл для поиска
                if (employees[i] == null) { // если массив в этой итерации пустой, сравнивать тут не с чем
                    check = false; // сохраняем состояние тестовой переменной
                } else { // иначе
                    if (employees[i].getName().equals(name) && employees[i].getState() == state) { // Проверяем совпадение в имени и отделе
                        check = true; // если всё совпало, сохраняем данные в проверочную переменную
                        idOfExistedEmployee = employees[i].getId(); // так же сохраняем id найденного сотрудника
                        break; // и прекращаем выполнение цикла
                    }
                }
            }
            // Проверяем, нашлась ли наша запись в массиве при поиске выше
