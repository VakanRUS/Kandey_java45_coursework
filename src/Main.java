public class Main {
    public static void main(String[] args) {
        ...
        // проверка работы метода, на предмет того, что он может удалить сотрудника в любом месте массива и вывести сообщение, если ячейка массива уже пустая

        // Вывод данных сотрудника с определённым id
        employeeBook.findEmployeeById(4);
        employeeBook.findEmployeeById(1);

        // проверка работы метода добавления сотрудника
        // на данный момент в массиве есть свободные места
        employeeBook.printAllEmployees();
        employeeBook.addEmployee("Инга", 52700.25, 1); // таких записей в массиве нет, спокойно добавляет
        employeeBook.addEmployee("Инга", 52700.25, 1); // сообщает, что такой сотрудник в массиве уже есть
        employeeBook.addEmployee("Ольга", 59100.51, 4); // таких записей в массиве нет, тоже спокойно добавляет
        employeeBook.addEmployee("Ольга", 59100.51, 4); // в массиве нет места, смысла проверять совпадение данных нет
    }
}